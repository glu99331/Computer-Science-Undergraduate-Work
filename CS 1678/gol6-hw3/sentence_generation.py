...
from rnn_datasets import ShakespeareDataset
from rnn_modules import GRUCell, LSTMCell
...
flags.DEFINE_integer('hidden_size', 130, 'Dimensionality for recurrent neuron.')
...
flags.DEFINE_integer('generation_length', 1000,
...
class SentenceGeneration(nn.Module):
  ...
  def __init__(self,
    ...
    self.embedding = nn.Embedding(num_embeddings=vocabulary_size, embedding_dim = embedding_dim, padding_idx=PADDING_TOKEN)
    self.rnn_model = self.rnn_module(input_size=embedding_dim, hidden_size=hidden_size, bias=bias)
  ...
  def forward(self, history, state=None):
    ...
    c_state = None
    batch_size, history__len, _ = data.shape
    for step in range(history__len):

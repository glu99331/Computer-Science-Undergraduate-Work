...
author: "Gordon Lu"
date: "Submission time: February 17, 2020 at 8:55PM"
output: pdf_document
---
...
```{r, solution_1a, fig.align = 'center', eval = TRUE}
...
### DISCUSSION:
Yes, there appears to be two apparent outliers. One that is an outlier on the upper bound, exceeding 10. The other one is an outlier on the lower bound, exceeding -10.
...
```{r, solution_1b, fig.align = 'center', eval = TRUE}
df_01 %>% ggplot(mapping = aes(x = x)) + 
  geom_histogram(bins = 25)
```
### DISCUSSION:
The distribution looks approximately symmetric and normal, with the exception of the outliers on both ends of the distribution. Most of the measurement values tend to be concentrated around 0.
...
quantile(df_01$x, probs = seq(0,1,0.05), na.rm = TRUE)
### calculate the empirical standard deviation of x
std_dev <- sd(df_01$x)

```

### DISCUSSION: The 5th percentile value of around -7.8 looks to be around the correct value based on the run chart and histogram. So too does the 95th percentile value of around 9.2.  
...
```{r, solution_1d, fig.align = 'center',  out.height = '40%', eval = TRUE}
...
### DISCUSSION:
The rolling averages seem to be the most variable around the beginning, the observations from approximately 0 to 50 seem to fluctuate greatly, whereas beyond 100 observations, it seems to narrow out, and the rolling average looks to steady out.
...
#### SOLUTION
After 5 observations, the sample average is equal to approximately 0.25.
After 45 observations, the sample average is equal to approximately 1.24.
...
We want to transform the bounded variable $\sigma$ to a new unbounded variable, and we want to fufill a constraint on bounds based on the given distribution, therefore tranforming kills two birds with one stone.  
...
$$
p(\mu, \varphi | \mathbf{x}) = \prod_{n=1}^{N} [normal(x_n | \mu, g^{-1}({\varphi})] \times p(\mu) \times p(g^{-1}(\varphi)) \times |\frac{d}{d\varphi}(g^{-1}(\varphi))|
$$
...
my_logpost <- function(unknowns, my_info)
{
  ...
  # Similarly could have done:
  
  # log_lik <- sum(-1/2 * log(2*pi) - 1/2 * log(lik_sigma^2) - (my_info$xobs - lik_mu)^2/(2*(lik_sigma^2)))
 ...
 log_prior_mu <- dnorm(x = lik_mu,
  ...
  # log_prior_mu <- -1/2 * log(2*pi) - 1/2 * log(my_info$tau_0^2) - (lik_mu - my_info$mu_0)^2/(2*(my_info$tau_0^2))
  
  log_prior_sigma <- log(my_info$sigma_rate) - (lik_sigma * my_info$sigma_rate)
...
```{r, solution_02f, fig.align = 'center', eval=TRUE}
viz_logpost_surface(log_post_inform_N05, param_grid)
```

### DISCUSSION:
For the tranformed unbounded $\varphi$ paremeter, values that are negative are ruled out. For the unknown mean $\mu$, the most probable values seem to be centered around -0.5.
No, the posterior uncertainty on $\mu$ does is not small relative to the prior uncertainty.
...
```{r, solution_02g_b, fig.align = 'center', eval=TRUE}
viz_logpost_surface(log_post_inform_N45, param_grid)
```

```{r, solution_02g_c, fig.align = 'center', eval=TRUE}
viz_logpost_surface(log_post_inform_N405, param_grid)
```

### DISCUSSION:
As the sample size increases, the log-posterior surface begins to narrow in, and look somewhat normal looking around the posterior mode.
The most probable values of $\varphi$ and $\mu$ do not seem to change, if not, it changes by a very small amount. 
The values that are ruled out have increased.
The uncertainty has also decreased, which can be seen by the amount of values that have been ruled out, the size of the contour is significantly smaller, reflecting this increase in certainty.
...
Yes, the identified optimal parameter values and the Hessian matrices are the same. No, there is nothing different besides the starting point.
The log-posterior surfaces only have a single peak, therefore, the results should be expected to be the same.
...
laplace_res_inform_N05$converge

laplace_res_inform_N45$converge

laplace_res_inform_N405$converge
...
post_samples_inform_N05 %>% ggplot(mapping = aes(x = sigma)) + geom_histogram()
post_samples_inform_N05 %>% ggplot(mapping = aes(x = mu)) + geom_histogram()


post_samples_inform_N45 %>% ggplot(mapping = aes(x = sigma)) + geom_histogram()
post_samples_inform_N45 %>% ggplot(mapping = aes(x = mu)) + geom_histogram()

post_samples_inform_N405 %>% ggplot(mapping = aes(x = sigma)) + geom_histogram()
post_samples_inform_N405 %>% ggplot(mapping = aes(x = mu)) + geom_histogram()
...
### DISCUSSION:
Numerically, the posterior mean value and the standard deviation at 5 samples are: $\mu$: -0.3697 and $\sigma$: 5.175.
The posterior mean value and the standard deviation at 45 samples are: $\mu$: 0.6407 and $\sigma$: 4.927.
The posterior mean value and the standard deviation at 45 samples are: $\mu$: 0.7044 and $\sigma$: 5.154.

The results of the above plots follow from the Central Limit Theorem. As the number of observations increases, the amount of noise or the standard deviation by definition.
As a result, the distributions begin to approximate a normal distribution.
...
post_prob_mu_N05 <- 1 - pnorm(0, mean = mean(post_samples_inform_N05$mu), sd = sd(post_samples_inform_N05$sigma))

post_prob_mu_N45 <- 1 - pnorm(0, mean = mean(post_samples_inform_N45$mu), sd = sd(post_samples_inform_N45$sigma))

post_prob_mu_N405 <- 1 - pnorm(0, mean = mean(post_samples_inform_N405$mu), sd = sd(post_samples_inform_N405$sigma))

prob_mu_vector <- c(post_prob_mu_N05, post_prob_mu_N45, post_prob_mu_N405)

prob_mu_vector
```

```{r}
post_quantile_lwr_sigma_N05 <- qnorm(0.05, mean = mean(post_samples_inform_N05$mu), sd = sd(post_samples_inform_N05$sigma))

post_quantile_uppr_sigma_N05 <- qnorm(0.95, mean = mean(post_samples_inform_N05$mu), sd = sd(post_samples_inform_N05$sigma))

post_quantile_lwr_sigma_N45 <- qnorm(0.05, mean = mean(post_samples_inform_N45$mu), sd = sd(post_samples_inform_N45$sigma))

post_quantile_uppr_sigma_N45 <- qnorm(0.95, mean = mean(post_samples_inform_N45$mu), sd = sd(post_samples_inform_N45$sigma))

post_quantile_lwr_sigma_N405 <- qnorm(0.05, mean = mean(post_samples_inform_N405$mu), sd = sd(post_samples_inform_N405$sigma))

post_quantile_uppr_sigma_N405 <- qnorm(0.95, mean = mean(post_samples_inform_N405$mu), sd = sd(post_samples_inform_N405$sigma))

quantile_vector <- c(post_quantile_lwr_sigma_N05, post_quantile_uppr_sigma_N05, post_quantile_lwr_sigma_N45, 
                     post_quantile_uppr_sigma_N45, post_quantile_lwr_sigma_N405, post_quantile_uppr_sigma_N405)
...
#### SOLUTION
The posterior probability that $\mu$ is positive for 5 observations is: 0.3995590.
The posterior probability that $\mu$ is positive for 45 observations is: 0.8904088.
The posterior probability that $\mu$ is positive for 405 observations is: 0.9999444.

The 0.05 quantile on $\sigma$ for 5 observations is: -2.7593428. The 0.95 quantile on $\sigma$ for 5 observations is: 2.0199059.
The 0.05 quantile on $\sigma$ for 45 observations is: -0.2170140. The 0.95 quantile on $\sigma$ for 45 observations is: 1.4985096.
The 0.05 quantile on $\sigma$ for 405 observations is 0.4045902. The 0.95 quantile on $\sigma$ for 405 observations is: 1.0041649.
...
df_02_low %>% ggplot(mapping = aes(x = x)) +
  geom_point(mapping = aes(y = y),
             size = 1)

df_02_high %>% ggplot(mapping = aes(x = x)) +
  geom_point(mapping = aes(y = y),
             size = 1)
...
### DISCUSSION:
Based on the form of the relatively low-diffuse data set, I can tell that the original function takes on the some sort of parabolic relationship. Most likely a even-power polynomial, most likely a quadratic one.
...
mod_low_8th <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) +
                    I(x^6) + I(x^7) + I(x^8), data = df_02_low)
...
mod_high_quad <- lm(y ~ x + I(x^2), data = df_02_high)

mod_high_8th <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + 
                     I(x^6) + I(x^7) + I(x^8), data = df_02_high) 
```
...
### DISCUSSION: 
Among the low noise case, the 8th order polynomial relationship has the highest r-squared value.
Among the high noise case, the 8th order polynomial relationship has the highest r-squared value.
...
mod_low_quad_fold_01 <- lm(y ~ x + I(x^2), data = low_noise_train_fold_01)

mod_low_quad_fold_02 <- lm(y ~ x + I(x^2), data = low_noise_train_fold_02)

mod_low_quad_fold_03 <- lm(y ~ x + I(x^2), data = low_noise_train_fold_03)

mod_low_quad_fold_04 <- lm(y ~ x + I(x^2), data = low_noise_train_fold_04)

mod_low_quad_fold_05 <- lm(y ~ x + I(x^2), data = low_noise_train_fold_05)
```

The 8th order polynomial fits should be specified below.  

```{r, solution_04f_b, eval=TRUE}
mod_low_8th_fold_01 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + 
                            I(x^6) + I(x^7) + I(x^8), data = low_noise_train_fold_01)

mod_low_8th_fold_02 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + 
                            I(x^6) + I(x^7) + I(x^8), data = low_noise_train_fold_02)

mod_low_8th_fold_03 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + 
                            I(x^6) + I(x^7) + I(x^8), data = low_noise_train_fold_03)

mod_low_8th_fold_04 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + 
                            I(x^6) + I(x^7) + I(x^8), data = low_noise_train_fold_04)

mod_low_8th_fold_05 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + 
                            I(x^6) + I(x^7) + I(x^8), data = low_noise_train_fold_05)
```
...
```{r}
#Calculate the RMSE of each of the folds from the 5-fold for the 2nd order polynomial folds of the low-noise training data set that has already been partitioned into folds.

mod_low_quad_train_fold_01_rmse <- modelr::rmse(mod_low_quad_fold_01, low_noise_train_fold_01)

mod_low_quad_train_fold_02_rmse <- modelr::rmse(mod_low_quad_fold_02, low_noise_train_fold_02)

mod_low_quad_train_fold_03_rmse <- modelr::rmse(mod_low_quad_fold_03, low_noise_train_fold_03)

mod_low_quad_train_fold_04_rmse <- modelr::rmse(mod_low_quad_fold_04, low_noise_train_fold_04)

mod_low_quad_train_fold_05_rmse <- modelr::rmse(mod_low_quad_fold_05, low_noise_train_fold_05)

#Send each fold into a vector of RMSEs from the 2nd order polynomial folds...

cv_low_quad_fold_train_rmse <- c(mod_low_quad_train_fold_01_rmse, mod_low_quad_train_fold_02_rmse, 
                                 mod_low_quad_train_fold_03_rmse, mod_low_quad_train_fold_04_rmse, 
                                 mod_low_quad_train_fold_05_rmse)
...
```{r}
#Calculate the RMSE of each of the folds from the 5-fold for the 8th order polynomial folds of the low-noise training data set that has already been partitioned into folds.

mod_low_8th_train_fold_01_rmse <- modelr::rmse(mod_low_8th_fold_01, low_noise_train_fold_01)

mod_low_8th_train_fold_02_rmse <- modelr::rmse(mod_low_8th_fold_02, low_noise_train_fold_02)

mod_low_8th_train_fold_03_rmse <- modelr::rmse(mod_low_8th_fold_03, low_noise_train_fold_03)

mod_low_8th_train_fold_04_rmse <- modelr::rmse(mod_low_8th_fold_04, low_noise_train_fold_04)

mod_low_8th_train_fold_05_rmse <- modelr::rmse(mod_low_8th_fold_05, low_noise_train_fold_05)

#Send each fold into a vector of RMSEs from the 8th order polynomial folds...

cv_low_8th_fold_train_rmse <- c(mod_low_8th_train_fold_01_rmse, mod_low_8th_train_fold_02_rmse, 
                                mod_low_8th_train_fold_03_rmse, mod_low_8th_train_fold_04_rmse, 
                                mod_low_8th_train_fold_05_rmse)
...
### DISCUSSION: 
The quadratic relationship has the lowest average RMSE among the 5-folds.
...
```{r}
#Calculate the RMSE of each of the folds from the 5-fold for the 2nd order polynomial folds of the low-noise test data set that has already been partitioned into folds.

mod_low_quad_test_fold_01_rmse <- modelr::rmse(mod_low_quad_fold_01, low_noise_test_fold_01)

mod_low_quad_test_fold_02_rmse <- modelr::rmse(mod_low_quad_fold_02, low_noise_test_fold_02)

mod_low_quad_test_fold_03_rmse <- modelr::rmse(mod_low_quad_fold_03, low_noise_test_fold_03)

mod_low_quad_test_fold_04_rmse <- modelr::rmse(mod_low_quad_fold_04, low_noise_test_fold_04)

mod_low_quad_test_fold_05_rmse <- modelr::rmse(mod_low_quad_fold_05, low_noise_test_fold_05)

#Send each fold into a vector of RMSEs from the 2nd order polynomial folds...

cv_low_quad_fold_test_rmse <- c(mod_low_quad_test_fold_01_rmse, mod_low_quad_test_fold_02_rmse, 
                                mod_low_quad_test_fold_03_rmse, mod_low_quad_test_fold_04_rmse, 
                                mod_low_quad_test_fold_05_rmse)
...
#Calculate the RMSE of each of the folds from the 5-fold for the 8th order polynomial folds of the low-noise test data set that has already been partitioned into folds.

mod_low_8th_test_fold_01_rmse <- modelr::rmse(mod_low_8th_fold_01, low_noise_test_fold_01)

mod_low_8th_test_fold_02_rmse <- modelr::rmse(mod_low_8th_fold_02, low_noise_test_fold_02)

mod_low_8th_test_fold_03_rmse <- modelr::rmse(mod_low_8th_fold_03, low_noise_test_fold_03)

mod_low_8th_test_fold_04_rmse <- modelr::rmse(mod_low_8th_fold_04, low_noise_test_fold_04)

mod_low_8th_test_fold_05_rmse <- modelr::rmse(mod_low_8th_fold_05, low_noise_test_fold_05)

#Send each fold into a vector of RMSEs from the 2nd order polynomial folds...

cv_low_8th_fold_test_rmse <- c(mod_low_8th_test_fold_01_rmse, mod_low_8th_test_fold_02_rmse, 
                               mod_low_8th_test_fold_03_rmse, mod_low_8th_test_fold_04_rmse, 
                               mod_low_8th_test_fold_05_rmse)
...
### DISCUSSION:
The quadratic relationship has the lowest average cross-validation RMSE on test splits.

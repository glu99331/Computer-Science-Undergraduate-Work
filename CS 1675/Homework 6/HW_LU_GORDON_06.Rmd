...
author: "Gordon Lu"
date: "Submission time: April 8, 2020 at 2:45AM EST"
output: pdf_document
---
...
```{r, solution_01a_a, eval=TRUE, fig.align = 'center'}
###
train_data %>% ggplot(mapping = aes(x = x)) +
...
```{r, solution_01a_b, eval=TRUE, fig.align='center'}
### 
train_data %>% ggplot(mapping = aes(x = x)) +
  geom_point(mapping = aes(y = y),
             size = 1) + geom_smooth(mapping = aes(y = y))
```

And then manually adjust the `span` parameter.  

```{r, solution_01a_c, eval=TRUE, fig.align = 'center'}
### 
train_data %>% ggplot(mapping = aes(x = x)) +
  geom_point(mapping = aes(y = y), size = 1) + 
  geom_smooth(mapping = aes(y = y), span = 0.2)
```
...
Your answer here, add as many equation blocks as you feel are necessary.  
...
### DISCUSSION:
The dimensionality of the $\mathbf{A}$ matrix is (N x 2) x (2 x H) = (N x H).
...
### DISCUSSION:
The dimensionality of the $\mathbf{f}$ vector is (N x 2) x (2 x H) x (H x 1) = (N x H) x (H x 1) = (N x 1).
...
### design matrix
Xmat <- model.matrix( ~ train_data$x, data = data.frame(x = train_data$x))
...
### DISCUSSION:
No, the optimal parameters are not the same between the two starting guesses. Unlike with linear and logistic regression, with neural networks, the initial guess has an impact on the parameters, which therefore impacted the hidden units. The neural network likelihood surface is multimodal, this means, unlike with linear and logistic regression, the likelihood consists of many local optima, and our optimization scheme can end up getting "stuck" at local optima. Therefore, the global optima and "best" parameter estimates can be missed!
...
$$
\frac{\partial{R_n}}{\partial{\alpha_k}} = \frac{\partial}{\partial{\alpha_k}}((y_n-f_n)^2)
$$
$$
\frac{\partial{R_n}}{\partial{\alpha_k}} = -2(y_n-f_n)\frac{\partial{f_n}}{\partial{\alpha_k}}
$$

$$
\frac{\partial{R_n}}{\partial{\alpha_k}} = -2(y_n-f_n)\frac{\partial}{\partial{\alpha_k}}(\sum_{n=1}^N(\alpha_k h_{n,k}))
$$
$$
\frac{\partial{R_n}}{\partial{\alpha_k}} = -2(y_n-f_n)h_{n,k}
$$
$$
...
$$
\frac{\partial}{\partial \alpha_k} \left(SSE\right) = \sum_{n=1}^{N}-2(\delta_n)h_{n,k}
$$

$$
\frac{\partial}{\partial \alpha_k} \left(SSE\right) = -2(\mathbf{h_{:,k}}^T\boldsymbol{\delta})
$$
...
$$
\mathbf{g_\alpha} = \left[\begin{array} 
{ccc}
-2(\mathbf{h_{:,1}}^T\boldsymbol{\delta})\\
-2(\mathbf{h_{:,2}}^T\boldsymbol{\delta})\\
...\\
-2(\mathbf{h_{:,k}}^T\boldsymbol{\delta})\\
\end{array}
\right]
$$

### DISCUSSION:
The dimensionality of this vector is (k x 1). Yes, I am indeed missing an output layer parameter. In particular, I am missing the derivative of the residual with respect to the $\alpha_0$ parameter.
...
$$
\frac{\partial R_n}{\partial \alpha_{dk}} = -2 \xi_{n_k}x_{n,d}
$$
...
$$
g_{n,k}^{'}(\eta) = (1-g(\eta))g(\eta) = g(\eta) - g^2(\eta)
$$

$$
\mathbf{G}_p = \mathbf{H} - (\mathbf{H} \circ \mathbf{H})
$$
...
backprop_grad_batch <- function(theta, my_info)
{
  ...
  da <- -2 * (t(cbind(c(1), H))) %*% as.matrix(d)
...
```
### DISCUSSION:
Since the output layer parameters are zero, the $\mathbf{A}_W$ matrix contains all zeros. Since the $\mathbf{A}_W$ matrix only has zeros, the $\mathbf{E}$ matrix also contains only zeros. Since the $\mathbf{E}$ matrix only has zeros, the $\mathbf{J}$ matrix also contains zero. This implies that there is no change to the hidden unit parameters. I.e., the parameters remain unchanged at zero.
...
### DISCUSSION:
The optimal parameter estimates of the back propagation call are fairly similar for some the parameter estimates of the finite difference call. However, in some of the estimates, the optimal parameter estimates of the back propagation call are vastly different than that of the finite difference call.
...
### DISCUSSION:
There are 34 parameters in the model with 11 hidden units.

Do we need to define a new design matrix?  

### DISCUSSION:
No, there is no need to define a new design matrix.
...
### DISCUSSION:
The run time for the analytic gradient functino is 50 times faster than the finite differences.
...
### DISCUSSION:
The best model is the one with one hidden layer and 5 nodes. On folds 1 - 5, the number of samples used were
199, 201, 201, 199, and 200 respectively. This model was also the model that performed the best during 
cross-validation performance. The $R^2$ value associated with this model is ~0.991
...
nnet_grid <- expand.grid(layer1 = c(seq(2, 14, 3)),
                         layer2 = c(seq(0, 6, 2)),
                         layer3 = 0)
```
...
### DISCUSSION:
The best model has one hidden layer, with 14 nodes.
...
```{r, viz_caret_cv_results_a, eval=TRUE, fig.align = 'center'}
ggplot(fit_nnet_cv) + theme_bw() + theme(legend.position = "top")
```

The custom plot with the standard errors.  

```{r, viz_caret_cv_results_b, eval=TRUE, fig.align = 'center'}
...
### DISCUSSION:
The model with 5 nodes in layer 1 and no nodes in layer 2 would be sufficient for this data.
...
author: "Gordon Lu"
date: "Submission time: March 8, 2020 at 6:40PM"
output: pdf_document
---
...
```{r, solution_01a, eval=TRUE, fig.align = 'center'}
...
### DISCUSSION:
Based on the scatter plot above, I would think there would be a strong linear relationship between the input and response variables.
...
### DISCUSSION:
No, we shouln't be concerned that the initial guess impacted the posterior results. This is because in linear models, there is always a unique maximum.
...
```{r, solution_01e_c, eval=TRUE, fig.align = 'center'}
...
### DISCUSSION:
The columns correspond to the number of samples. In this case, 2500 samples.
...
### DISCUSSION:
We consider the mean trend in calculating the error with the response rather than the predicated response values 
since the mean trend is the exact point where the model will be centered around. If predicted response values
were considered instead, it's likely than there will be many outliers, which will in effect throw off the 
error measurement.
...
### DISCUSSION:
The length of error_01_post_01 is 50. 
The three MAE values are not the same. They are not the same, since the posterior predicted mean trends are calculated from psuedo-random sample data. 
In calculating the MAE, we are averaging over the absolute value of the error of the predicted value against the actual value. 
...
### DISCUSSION:
The MAE is uncertain due to the fact that we're unsure of how well our random synthetic data resembles the
actual data and how well the model actually fits that data.
...
```{r, solution_03a, eval=TRUE, fig.align = 'center'}
train_02 %>% ggplot() +
...
### DISCUSSION:
There are 150 rows in the training matrices, and 50 rows in the testing matrices.
...
```{r, solution_03e, eval=TRUE, fig.align = 'center'}
...
```{r, solution_03e_b, eval=TRUE, fig.align = 'center'}
...
### DISCUSSION:
Based on the visualizations of the plots above, it appears that all models with J > 5 will overfit the data. This implies that somewhere around J = 5 is the better model.
...
```{r, solution_03f_b, eval=TRUE, fig.align = 'center'}
...
### DISCUSSION: 
Based on the above visualization, the 5th order spline is the best model.
...
The conclusions yielded the same result. Both comparisons point to the fact that the J = 5 model is the best option to model the data. 
...
#### SOLUTION
$$
p(\boldsymbol{y}\mid\boldsymbol{\eta}) = \prod_{n = 1}^{N}(logit^{-1}({\eta_n}))^{y_n} * (1 - logit^{-1}(\eta_n))^{1-y_n}
$$
$$ 
\eta_n = \boldsymbol{x_n}:\boldsymbol{\beta}
$$
...
### DISCUSSION:
The initial guess shoud have no impact, since the surface is unimodal. The betas both contain zero in the
middle 95% interval.
...
### DISCUSSION:
The linear model is supported more than the cubic.
...
### DISCUSSION:
We cannot directly use the ‘generate_lm_post_samples()’ function since we don't know immediately
know how many betas we will have.
...
### DISCUSSION:
There are 100 rows per summary.
...
```{r, solution_05d_c, eval=TRUE, fig.align = 'center'}
...
### DISCUSSION:
The linear model seems to be more in line with the observations. 
...
### DISCUSSION:
There are 40 true-negatives, 13 false-negatives, 14 false-positives, and 33 true-positives.
...
### DISCUSSION:
There are 38 true-negatives, 16 false-negatives, 12 false-positives, and 34 true-positives.
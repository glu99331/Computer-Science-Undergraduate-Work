...
author: "Gordon Lu"
date: "Submission time: January 17, 2020 at 1:30AM"
output: pdf_document
---
...
```{r, solution_01a, fig.align='center'}
...
```{r, solution_01b, fig.align='center'}
...
```{r, solution_01c, fig.align='center'}
...
```{r, solution_01d, fig.align='center'}
iris %>% ggplot(mapping = aes(x=Sepal.Length)) + 
...
```{r, solution_01e, fig.align='center'}
iris %>% ggplot(mapping = aes(x=Sepal.Length)) + 
...
```{r, solution_03f, eval=TRUE, fig.align='center'}
my_iris %>% 
...
```{r, solution_04c, eval=TRUE, fig.align='center'}
iris_group <- my_iris %>% 
...
```{r, solution_04e, eval=TRUE, fig.align = 'center', out.height = '40%'}
iris_group %>% 
  ggplot(mapping = aes(x = avg_sepal_length)) +
...
```{r, solution_04f_a, eval=TRUE, fig.align='center', out.height = '30%'}
iris_group %>% 
  ggplot(mapping = aes(x = avg_sepal_length)) +
...
```{r, solution_05b, fig.align='center', out.height = '40%'}
...
```{r, solution_05c, eval=TRUE, fig.align='center'}
...
```{r, solution_05d, fig.align='center'}
diamonds %>% ggplot(mapping = aes(x = color, y = price)) + 
...
```{r, solution_05e, fig.align='center'}
diamonds %>% ggplot(mapping = aes(x = color, y = price)) + 
...
```{r, solution_05f, fig.align='center'}
...
The set of diamonds that have an Ideal Cut, and IF Clarity, whose carat is above the average carat, as the color classification increases (alphabetically), the average price will generally decrease. 
...
$$ 
\frac{\partial{f}}{\partial{x_1}} = ab x_{1}^{b-1}
$$
...
$$ 
\frac{\partial{f}}{\partial{x_2}} = cd x_{2}^{d-1}
$$
...
$$ 
\frac{\partial^2{f}}{\partial{x_2}^2} = ab (b-1) x_{1}^{b-2}
$$
...
$$ 
\mathbf{x}^T\mathbf{x} = x_{1}^2 + x_{2}^2 +...+x_{N}^2
$$
...
$$ 
\mathbf{x}^T\mathbf{x} = \sum_{n=1}^N(x_{n}^2)
$$
...
$$
Inner: \mathbf{x}^T\mathbf{x} = \left(1 \times 1 \right) 
$$
$$
Outer:\mathbf{x}\mathbf{x}^T = \left(N \times N \right)
$$

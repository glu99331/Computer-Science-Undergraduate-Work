...
author: "Gordon Lu"
date: "Submission time: February 1, 2020 at 8:20PM"
output: pdf_document
---
...
$$ 
\mu_{ML} = \frac{M}{N} =\frac{1}{N} \sum_{n=1}^N{(x_N)} = \frac{H}{AB}
$$
...
$$ 
\log{p(H|\mu,AB)} \propto Hlog{(\mu)} + (AB-H)log{(1-\mu)}
$$
...
### DISCUSSION: Yes, the MLE does correspond to the same location as it did with the un-normalized log-likelihood.  
...
player_atbats <- 602
true_mu <- 0.3
hit_prob <- exp(log_binom_pmf(player_hits, player_atbats, true_mu))
...
### your code here
abs(pbinom(q=165, size=602, prob=0.3) - pbinom(q=205, size=602, prob=0.3))
```
...
$$ 
log[p(\mu|a,b)] \propto (a-1)log(\mu) + (b-1)log(1-\mu)
$$
...
$$ 
log[p(\mu|AB,H,a,b)] \propto log[Beta(\mu|a,b)] + Hlog(\mu) + (AB-H)log(1-\mu)
$$

$$ 
log[p(\mu|a,b)] \propto (a-1)log(\mu) + (b-1)log(1-\mu) +Hlog(\mu) + (AB-H)log(1-\mu) 
$$
$$ 
log[p(\mu|a,b)] \propto (a - 1 + H)log(\mu) + (b - 1 + AB - H)log(1-\mu)
$$
$$ 
log[p(\mu|a,b)] \propto log[Beta(\mu|a + H, b + AB - H)]
$$
$$ 
a_{new} = a + H
$$
...
```{r, solution_03c_b}
prior_mean <- (a_prior-1)/(b_prior + a_prior - 2)
prior_mean
```
...
post_mean_start <- (a_post_start-1)/(b_post_start+a_post_start-2)
post_mean_player <- (a_post_player-1)/(b_post_player + a_post_player -2)
post_mean_start
post_mean_player
```

### DISCUSSION:
The posterior_mean_start is approximately 0.04 higher than the corresponding MLE at 30 hits.
The posterior_mean_player is approximately 0.03 lower than the corresponding MLE for all hits (602 hits).
...
```
I feel fairly confident in saying that the player is a good hitter based on data from the complete season. 
## Problem 4
...
$$
$$
log(p \left(\sigma \mid \mathbf{x}, \mu\right)) \propto \sum_{n=1}^{N} log\left(\mathrm{normal}\left( x_n \mid \mu, \sigma \right) \right) +  log(\mathrm{Exp}\left( \sigma \mid \lambda \right))
$$

$$
log(p \left(\sigma \mid \mathbf{x}, \mu\right)) \propto -\lambda \sigma + \sum_{n=1}^{N} log\left(\mathrm{normal}\left(x_n \mid \mu, \sigma \right) \right)
$$

$$
log(p \left(\sigma \mid \mathbf{x}, \mu\right)) \propto -\lambda \sigma + \sum_{n=1}^{N} log(\frac{1}{\sigma\sqrt{2\pi}}exp(-\frac{(x_n-\mu)^{2}}{2\sigma^{2}})
$$

$$
log(p \left(\sigma \mid \mathbf{x}, \mu\right)) \propto -\lambda \sigma + Nlog(\frac{1}{\sigma \sqrt{2\pi}}) 
+ \sum_{n=1}^{N}(-\frac{(x_n-\mu)^{2}}{2\sigma^{2}})
$$

$$
log(p \left(\sigma \mid \mathbf{x}, \mu\right)) \propto -\lambda \sigma + Nlog(1) - Nlog(\sigma\sqrt{2\pi}) 
+ \sum_{n=1}^{N}(-\frac{(x_n-\mu)^{2}}{2\sigma^{2}})
$$

$$
log(p \left(\sigma \mid \mathbf{x}, \mu\right)) \propto -\lambda \sigma - Nlog(\sigma) - Nlog(\sqrt{2\pi}) 
+ \sum_{n=1}^{N}(-\frac{(x_n-\mu)^{2}}{2\sigma^{2}})
$$

$$
log(p \left(\sigma \mid \mathbf{x}, \mu\right)) \propto -\lambda \sigma - Nlog(\sigma) 
+ \sum_{n=1}^{N}(-\frac{(x_n-\mu)^{2}}{2\sigma^{2}})
$$
...
{
  log_post <- -1*(lambda*sigma) - (N*log(sigma))-((N/(2*(sigma**2)))*v)
  return(log_post)
...
```{r, solution_04d, eval=TRUE, fig.align='center'}
tibble::tibble(
...
DISCUSSION:
The log-posterior looks somewhat like the log curve, but also somewhat like an angled parabolic curve, but nonetheless, it looks irregular for a parabola. The log-posteriror appears to have a singular mode.
...
$$
\frac{d}{d\varphi} g^{-1}(\varphi) = exp(\varphi)
$$
...
```{r, solution_04g, eval=TRUE, fig.align='center'}
tibble::tibble(

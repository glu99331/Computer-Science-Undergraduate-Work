...
public class Biconnected {
    ...
    private void dfs(Graph G, int u, int v) {
        ...
        pre[v] = cnt++;
        low[v] = pre[v];
        for (UnweightedEdge uEdge : G.adj(v)) {
            int w = uEdge.retrieveEnd();
            if (pre[w] == -1) {
                children++;
                dfs(G, v, w);
    ...
    public boolean isArticulation(int v, int toIgnore) { 
        articulation[toIgnore] = false;
        return articulation[v]; 
    }
    public boolean isArticulation(int v) { 
        System.out.println(v + " is an Articulation Point? " + articulation[v]);
        return articulation[v]; 
    }
    public boolean hasArticulationPoint(int toIgnore)
    {
        System.out.println("Ignore the hoe: " + toIgnore);
        articulation[toIgnore] = false;

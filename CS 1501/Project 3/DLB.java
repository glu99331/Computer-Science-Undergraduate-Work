...
public class DLB<Value> 
{
    ...
    private Node<Value> rootNode; //Root node of DLB
    ...
    private static class Node<Value>
    {
        ...
        private Node<Value> childNode, siblingNode;        // child and sibling references
        /**************************************
    ...
    /*************************************
     * Does the DLB have a word in it?
     * 
     * @return whether or not the root
     * node has a childNode
     ************************************/
    public boolean isChildless() 
    {
        return rootNode.childNode == null ? true : false;
    }
     ...
     *********************************************************/
     public boolean put(String key, Value val)
     {
        ...
        Node<Value> currentNode = putPrimaryHelper(key);

        if(currentNode.c == TERMINATOR_CHAR)
        {
            //Insert value at the terminating character
            currentNode.val = val;
            return true;
        }
        return false;
     ...
     *********************************************************/
     public Node<Value> putPrimaryHelper(String key)
     {
        key += TERMINATOR_CHAR; //Append the terminating character to the string to be inserted to indicate the end of a word.
        //Have a pointer to the rootNode to navigate through the DLB
        Node<Value> currNode = rootNode;
        //Loop through all the characters in the String...
        for(int i = 0; i < key.length(); i++)
        {
            //Case I: If the root node is null, just create the first node...
            if(putSecondaryHelper("CHECK_ROOT", currNode, key.charAt(i)))
            {
                //Create rootNode:
                rootNode = new Node(key.charAt(0));
                currNode = rootNode;
            }
            ...
            else if(putSecondaryHelper("ADD_CHILD", currNode, key.charAt(i)))
            {
                currNode = addChild(currNode, key.charAt(i));
            }
            //Case III: If the child is not null, traverse until we get to a node whose child is null...
            else if(putSecondaryHelper("TRAVERSE_CHILD", currNode, key.charAt(i)))
            {
            ...
            else if(putSecondaryHelper("ADD_SIBLING", currNode, key.charAt(i)))
            {
                currNode = addSibling(currNode, key.charAt(i));
            }
            ...
            else if(putSecondaryHelper("TRAVERSE_SIBLING", currNode, key.charAt(i)))
            {
     ...
     ***********************************************************/
     public boolean searchPrefix(String prefix)
     {
        ...
        Node<Value> currentNode = searchPrefix(rootNode, prefixBuilder, prefix, 0); //At this point, we're assuming that the DLB contains something in it!
     ...
     **********************************************************************************/
     public Node<Value> searchPrefix(Node<Value> current, StringBuilder sb, String val, int char_pos)
     {
        //Return wherever currNode ends up at:
        ...
        else
        {
            ...
            else if(current.c == TERMINATOR_CHAR && current.siblingNode != null)
            {
                current = searchPrefix(current.siblingNode, sb, val, char_pos);
            }
        }
    ...
    {
        prefixBuilder = new StringBuilder(); //To compare word searched in DLB to the word passed in
        Node<Value> currentNode = searchHelper(rootNode, prefixBuilder, key, 0); //At this point, we're assuming that the DLB contains something in it!
        if(prefixBuilder.toString().equals(key) && currentNode.c == TERMINATOR_CHAR && currentNode.siblingNode == null)
        {
            return true;
        }
        return false;
     ...
     * @return The node we end up at as a result of a single search
     **********************************************************************************/
    public Node<Value> searchHelper(Node<Value> current, StringBuilder sb, String val, int char_pos)
    {
        //Base Case: Current node has value of Terminator Character, String passed matches
        //String built up, and not a prefix!
        //Base Case: Prefix matches passed in string
        if(sb.toString().equals(val) && current.c == TERMINATOR_CHAR && current.siblingNode == null)
        {
            return current;
        }
    ...
    {
        //Sanity check, if the query is empty, then simply return null.
        if(key.length() == 0)
        {
            return null;
        }
        //Navigate to the corresponding terminating node associated with the key:
        prefixBuilder = new StringBuilder(); //To compare word searched in DLB to the word passed in
        Node<Value> currentNode = searchHelper(rootNode, prefixBuilder, key, 0); //At this point, we're assuming that the DLB contains something in it!
        if(prefixBuilder.toString().equals(key) && currentNode.c == TERMINATOR_CHAR && currentNode.siblingNode == null)
        {
        ...
        {
            //Otherwise return null to indicate there is no value associated with the given key
            return null;
        }
    }
     ...
     **************************************************************/
    public Node<Value> navigateNode(Node<Value> current, String val, int char_pos)
    {
    ...
    {
        Node<Value> currentNode = rootNode; //We're assuming at this point that the DLB contains something in it.
     ...
     *************************************************************/
    private Node<Value> addChild(Node<Value> current, char letter)
    {
     ...
     ***********************************************************/
    public Node<Value> traverseChild(Node<Value> current)
    {
        return current.childNode;
    }
     ...
     *************************************************************/
    public Node<Value> addSibling(Node<Value> current, char letter)
    {
     ...
     ***********************************************************/
    public Node<Value> traverseSibling(Node<Value> current, int index)
    {
        return current.siblingNode;
    }
     ...
     ******************************************************************/
    public boolean putSecondaryHelper(String sentinelKey, Node<Value> currNode, char letter)
    {
        switch(sentinelKey)
        {
            //Does the root exist?
            ...
            case "ADD_CHILD":
                return (currNode.c != TERMINATOR_CHAR && currNode.childNode == null) ? true : false;
     ...
     ******************************************************************/
    public boolean traversalHelper(String sentinelKey, Node<Value> startingNode)
    {
        switch(sentinelKey)
        {
            //Need to traverse sibling and at terminator, indicates that we have a prefix to another word.
            case "ENCOUNTER_PREFIX":
                return (startingNode.c == TERMINATOR_CHAR && startingNode.siblingNode != null) ? true : false;
     ...
     * same prefix.
     **********************************************************************/
    public ArrayList<String> appendPrefix(StringBuilder anchorPoint, StringBuilder base, Node<Value> currentNode, ArrayList<String> prefixList)
    {
     ...
     * same prefix.
     *********************************************************************/
    public ArrayList<String> recursiveTraverseChild(StringBuilder base, Node<Value> currentNode, ArrayList<String> prefixList)
    {
        base.append(currentNode.c);
     ...
     * same prefix.
     ***********************************************************************/
    public ArrayList<String> recursiveTraverseBoth(StringBuilder anchorPoint, StringBuilder base, Node<Value> currentNode, ArrayList<String> prefixList)
    {
